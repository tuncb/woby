...
void printCommandLineHelp()
{
    std::printf(
        ...
        "    $instances = & $exe ctl instances --json | ConvertFrom-Json\n"
        "    if ($LASTEXITCODE -ne 0) { throw 'Instance discovery failed' }\n"
        "    $ready = $instances | Where-Object { $_.id -eq $id -and $_.ready }\n"
        "    if ($ready) { break }\n"
        "    if ((Get-Date) -ge $deadline) { throw 'Viewer startup timed out' }\n"
        ...
        "Out-String (raw JSON) or ConvertFrom-Json (parsed JSON) to wait for completion\n"
        "before reading $LASTEXITCODE. This also applies when assigning output to a variable.\n"
        "In Python, use subprocess.Popen for the viewer and subprocess.run for ctl.\n"
        "Do not wait for viewer exit or capture its streams to completion before using ctl.\n"
        "Poll instances for your matching id with ready=true; an empty array or ready=false\n"
        "means keep waiting until your deadline. A running process alone is not readiness.\n"
    ...
    std::printf(
        ...
        "Summary diagnostic arrays show the first 100 entries; full counts and truncation flags remain.\n"
        "analysis findings ANALYSIS_ID --side a|b --detector KEY pages current retained data.\n"
        "  --offset N starts at 0; --limit N is 1-100 (default 100). Follow nextOffset until null.\n"
        "  --collection PATH defaults to /findings. All path indexes are ZERO-based.\n"
        "  Nested examples: /findings/100/members, /findings/0/incidentFaces,\n"
        "  /findings/0/endpoints/0/sourcePoints, /findings/0/vertices/120/pointReferences.\n"
        "  Other arrays include /sources, /affectedFaces, /boundaryRegions and fin face/edge lists.\n"
        "  Nested lists inside each page remain bounded; page their collection paths separately.\n"
        "  Reuse the returned --revision TEXT for subsequent pages; changed results are rejected.\n"
        "  Run the detector first if results are not current. focus --index remains ONE-based.\n"
        "analysis export ANALYSIS_ID --path PATH waits for current results, then starts a\n"
        "  background full JSON export, including every retained finding and nested member.\n"
        "  CLI paths may be relative; RPC paths must be absolute. Parent directory must exist.\n"
        "  Existing files are never overwritten. Only one export runs per viewer.\n"
        "  analysis export-status reports export.state and export.entriesWritten (nested entries included).\n"
        "  analysis export-cancel requests cancellation; poll export-status for completion.\n"
        "  Completed exports publish the file; canceled/failed jobs clean their temporary files.\n"
        "  allRetainedResults means nothing was omitted from export; detectionComplete can\n"
        "  still be false for partial, unavailable, unchecked, canceled or failed detectors.\n"
        "  Export uses a snapshot; later scene edits do not alter it. Status retains only the latest job.\n"
        "analysis set ANALYSIS_ID --intersection-pair-limit N --intersection-candidate-limit N\n"
        "  saves per-side detection budgets (defaults 10000 pairs / 1000000 candidate tests).\n"
        "  Each accepts 0-2147483647; 0 explicitly means unlimited for that budget.\n"
        "  Use analysis run ANALYSIS_ID --detector self_intersections after changing manual checks.\n"
        "  Dense overlaps can require quadratic time/storage; analysis cancel stops detection.\n"
        "  JSON reports pairLimit, candidateLimit, detectionTruncated and truncationReason\n"
        "  (pair_limit/candidate_limit). Partial exact counts are null; knownCount is retained.\n"
        "  Pagination/export cannot recover pairs omitted by a detection budget.\n"
